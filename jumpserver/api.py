...
from ops_auth.auth import User
from ops_auth.auth import UserGroup
...
def get_asset_info(asset,user = None):
    """
    获取资产的相关管理账号端口等信息
    """
    ...
    try:
        ...
        ####get asset role for this username
        ####demo code
        info["role"] = asset.role
    except TypeError:
        info['port'] = 22
...
def get_role_key(user, role):
    ...
    user_role_key_path = os.path.join(user_role_key_dir, '%s.id_rsa' % (role))
    mkdir(user_role_key_dir, mode=777)
    if not os.path.isfile(user_role_key_path):
        with open(os.path.join(user_role_key_path)) as fk:
    ...
    def _deco(func):
        def __deco(request, *args, **kwargs):
            ...
            if role == 'admin':
                ...
                if request.user.web_role == 'CU':
                    return HttpResponseRedirect("/")
            elif role == 'super':
                # if request.session.get('role_id', 0) < 2:
                if request.user.web_role in ['CU', 'GA']:
...
def is_role_request(request, role='user'):
    ...
    if request.user.web_role == role_all.get(role, 'CU'):
        return True
    else:
        return False
