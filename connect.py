...
from jumpserver.api import ServerError, User, get_object, mkdir, get_asset_info
from jumpserver.api import logger, Log, TtyLog, get_role_key, CRYPTOR, bash, get_tmp_dir
from jperm.perm_api import gen_resource, get_group_asset_perm, get_group_user_perm, user_have_perm, Rule, Asset, AssetGroup
...
class Tty(object):
    ...
    def get_connect_info(self):
        ...
        """
        asset_info = get_asset_info(self.asset,self.user)
        role_key = get_role_key(self.user, self.role)
        role_pass = CRYPTOR.decrypt('')
        ...
        try:
            ...
            if role_key and os.path.isfile(role_key):
                try:
                    ...
                    logger.warning(u'使用ssh key %s 失败, 尝试只使用密码' % os.path.basename(role_key))
                    pass
...
class Nav(object):
    ...
    def print_nav():
        ...
        打印提示导航
        """
        msg = """\n\033[1;32m###   欢迎使用ofo 运维堡垒机   ### \033[0m
    ...
    def get_asset_group_member(self, str_r):
        ...
        if gid_pattern.match(str_r):
            ...
            # 获取资产组包含的资产
            if gid > len(self.user_perm["asset_group"]):
            ...
            for index, asset in enumerate(self.search_result):
                ...
                try:
                    print line % (index, asset.ip, asset_info.get('port'),
                                  self.truncate_str(asset.hostname), ",".join(role).replace("'", ''), asset.comment)
                except:
                    print line % (index, asset.ip, asset_info.get('port'),
                                  self.truncate_str(asset.hostname), ",".join(role).replace("'", ''), '')
            print
    ...
    def try_connect(self):
        try:
            ...
            print('%s Connecting %s@%s ...' % (login_user.username,role,asset.hostname))
    ...
    def print_asset_group(self):
        ...
        打印用户授权的资产组
        """
        user_asset_group_all = self.user_perm.get('asset_group', [])
        color_print('[%-3s] %-20s %s' % ('ID', '组名', '备注'), 'title')
        for asset_group in sorted(user_asset_group_all):
